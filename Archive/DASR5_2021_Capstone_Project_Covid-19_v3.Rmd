...
We will use Merged Dataset for predicting average sentiment based on daily ICU, Confirmed, Hospitalised and Daily Deaths cases. 
...
# 
# Second, it appears that the relationships between Daily deaths regarding average sentiment is different depending on daily confirmed cases.
...
#Daily death
merged_dataset %>% summarise(sd(Daily.Deaths))

grid_group3 <- merged_dataset %>% 
  data_grid(Intensive.Care.Unit..ICU. , Daily.Deaths = c(-1.26, 0.00, 1.26),
...
merged_dataset %>% summarise(sd(Daily.Confirmed))

grid_group4 <- merged_dataset %>% 
  data_grid(Intensive.Care.Unit..ICU. , Daily.Confirmed = c(-1.26, 0.00, 1.26),
...
2.	The relationship between daily average sentiment and daily ICU cases depends on daily cases and daily death. It appears that if there is a high death with lower daily ICU cases, the average sentiment will be lower. However, if there is a higher ICU cases with high death rate, the average sentiment is better. As for daily cases, be it low or high daily cases, the result is constant for the average sentiment.
...
There are two main segments for this project which is covid cases and twitter mining. Both of us have been involved in these two parts and in fact we met up twice a week for the last month just to work on this project. Yun Xuan focus point is on twitter minning and to analyse the sentiment whereas Alex is more on Covid cases and modeling. Having said that and as mentioned above, there is overlapping of effort from both of us as we have working together to put the whole project gel together.